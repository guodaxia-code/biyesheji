$(function(){
	...
	$(".num .sub").click(function(){
		...
		if(num<=1){
			...
			$.get("/store/cart/updateProductNumberFromCart",{"pid":pid,"pcount":num},function (data) {
	...
	$('.del').click(function(){
		...
		if($(this).parent().parent().hasClass("th")){
			var pid=$(this).parent().parent()[0].children[2].children[0].children[1].value;
			$(".mask").show();
			$(".tipDel").show();
			...
			$('.cer').click(function(){
				...
				$(".th").eq(index).remove();
				$.get("/store/cart/removeProductFromCart",{"pid":pid},function (data) {
					console.log("删除商品成功")
				});
			...
			if($(".th input[type='checkbox']:checked").length==0){
				...
				$('.cer').click(function(){
					$(".th input[type='checkbox']:checked").each(function(j){
							var pid=$(this).parents(".th")[0].children[2].children[0].children[1].value;
							console.log(pid)
						index = $(this).parents('.th').index()-1;
						$(".th").eq(index).remove();
						console.log();
